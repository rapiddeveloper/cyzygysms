...
//   }
  ...
  let setting = (()=>{
    return settingsStore.settings.find((setting) => setting.kind === props.payload?.settingKind);
  ...
  return (
    <ActionSheet>
      <Stack space={constants.actionSheetVerticalSpacing} style={[styles.container, {backgroundColor: theme.colors.background}]}>
       <Text style={[theme.typography.h6Headline, {textTransform: 'capitalize', color: theme.colors.onBackground}]}>Choose {setting.title}</Text>
       <Bleed horizontal={4}>
       <Divider  />
       </Bleed>
        {setting.values.map((option, index: number) => (
          <Stack key={index} horizontal align={'center'} space={4} style={styles.option}>
            <Text style={[theme.typography.subtitle1, {textTransform: 'capitalize', color: theme.colors.onBackground}]}>{option}</Text>
            <AdvancedCheckbox
