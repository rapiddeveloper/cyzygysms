...
const AddStudent = () => {
  ...
  const studentProfileFormStore = useStudentProfileFormStore(useShallow((state) => ({
  ...
  const handleSubmit = async (submittedData: StudentProfileFormData) => {
    ...
    await studentProfileFormStore.createProfile(submittedData)
   }


  return (
    <AddStudentView onSubmit={(submittedData)=>handleSubmit(submittedData)} onPhotoUpload={(selectedPhotoFileURL)=> handlePhotoUpload(selectedPhotoFileURL)} />
  )
}
